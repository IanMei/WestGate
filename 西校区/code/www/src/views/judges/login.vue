    ...
    <div class="register pc">
      ...
      <div class="register_gary">
        <div class="denglu2">
          ...
          <div class="jilao">
            <input type="checkbox"
                   v-model="form.remember" />已阅读并同意<router-link to="/service">《服务协议》</router-link>
          </div>
...
export default {
  ...
  created () {
    let self = this
    self.$store.commit('setTab', 7)
    api
      .GET('/judges/Online')
      .then(function (res) {
        if (res === 1) {
          self.$router.replace('/judges/mine')
        }
      })
  ...
  methods: {
    doLogin () {
      ...
      self.loading = true
      api
        .POST('/judges/Login', self.form)
        .then(function (res) {
          util.cookies.set('j-sessionid', res)
          setTimeout(function () {
            self.loading = false
            self.$router.push('/judges/mine')
          }, 300)
        })
        .catch(function (error) {
          console.log(error)
