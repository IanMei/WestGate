...
import { checkMenus, checkHeaderMenus } from '@/menu'
...
export default {
  ...
  created () {
    var ck = util.cookies.get('menus')
    const menus = checkMenus(ck)
    const headerMenu = checkHeaderMenus(ck)
    // 设置顶栏菜单
    this.$store.commit('d2admin/menu/headerSet', headerMenu)
    // 设置侧边栏菜单
