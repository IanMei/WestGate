  ...
  actions: {
    ...
    async close ({ state, commit, dispatch }, { tagName }) {
      ...
      if (isCurrent) {
        ...
        //   }
        // }
        var sorlist = JSON.parse(JSON.stringify(state.opened))
        if (sorlist.length > 0) {
          const index = state.opened.findIndex(page => page.fullPath === tagName)
          sorlist.splice(index, 1)
          sorlist.sort((a, b) => b.tamp - a.tamp)
          const len = state.opened.length
          for (let i = 0; i < len; i++) {
            if (state.opened[i].fullPath === sorlist[0].fullPath) {
    ...
    async setTitle ({ state, commit, dispatch }, { tagName, title }) {
      const len = state.opened.length
      for (let i = 0; i < len; i++) {
        if (state.opened[i].fullPath === tagName) {
          state.opened[i].meta.title = title
          break
        }
      }
    },
