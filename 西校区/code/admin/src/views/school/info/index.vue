    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column width="380"
        ...
        <template slot-scope="scope">
          ...
          {{ scope.row.city_name}}
          {{ scope.row.disyrict_name}} -->
          {{ scope.row.address}}
        </template>
      </el-table-column>
      <el-table-column prop="stu_qty"
                       width="100"
                       label="学生人数"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="recruit_qty"
                       width="100"
                       label="招生人数"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="tas_rate"
                       width="100"
                       label="师生配比"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="ft_qty"
                       width="100"
                       label="外教人数"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="状态"
        ...
        <template slot-scope="scope">
          <cms-state ref="comswitch"
                     ...
                     v-bind:action="'/school/state?recordId=' + scope.row.id"></cms-state>
        </template>
      </el-table-column>
       <el-table-column label="推荐"
        ...
        <template slot-scope="scope">
          <cms-state ref="comswitch"
                     v-model="scope.row.enabled"
                     v-bind:action="'/school/hot?recordId=' + scope.row.id"></cms-state>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          ...
          <el-button icon="el-icon-star-on"
                     size="mini"
                     type="success"
                     v-on:click="handlePoint(scope.$index, scope.row)"></el-button>
          <el-button size="mini"
                     type="primary"
                     v-on:click="handleHonor(scope.$index, scope.row)">荣誉</el-button>
          <el-button size="mini"
                     type="warning"
                     v-on:click="handleFacilities(scope.$index, scope.row)">设施</el-button>
...
export default {
  ...
  methods: {
    ...
    handleFacilities (index, row) {
      // 编辑row
      const that = this
      that.$refs.facilitiesForm.show(row.id)
    },
